...
Users can upload a *.hob_out file from the MODFLOW Head Observation (HOB) package or a *.res file generated by the automatic calibration tool PEST.

The app processes measured and simulated data (e.g. heads or discharges), computes residuals, and evaluates calibration performance using statistical measures such as ME, MAE, and RMSE. Results are visualized in a scatter plot where points are colored according to their residuals. For data originating from *.res files, the opacity of the plotted points reflects the observation weights, if available.

The resulting scatter plot can be saved and downloaded for further analysis or documentation.
"""
)
